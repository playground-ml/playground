...
async def run(
    ...
    if batch_instance == 1:
        result = await _run_llm_watched(env_factory, env_kwargs, agent, model_cfg)
        return [result]
...
async def _run_human(env_factory, env_kwargs: dict) -> dict:
    ...
    while True:
        step_index += 1
        display.print_observation(observation, step_index)
        ...
        if result.done:
            success = result.success
            display.print_observation(observation, step_index + 1)
...
async def _run_llm_watched(env_factory, env_kwargs: dict, agent: LLMAgent, model_cfg: dict) -> dict:
    env: GameEnv = env_factory(**env_kwargs)
    ...
    result = await _episode_loop(env, agent, system_prompt, writer, verbose=True)
...
async def _episode_loop(
    ...
    while True:
        ...
        agent_result: AgentResult = await agent.call(messages, env.action_schema)
        step_elapsed_ms = int((time.perf_counter() - t_step) * 1000)

        if verbose:
            display.print_llm_action(
                agent_result.raw_response,
                agent_result.parsed_action,
